return {
  ...
  dependencies = {
    ...
    "saadparwaiz1/cmp_luasnip",
    {
      "Leiyi548/cmp_im_zhh",
      keys = {
        { "<C-.>", "<Cmd>lua require('cmp_im').toggle()<CR>", desc = "虎码启动!" },
      },
    },
  },
  opts = function()
    ...
    return {
      enabled = function()
        ...
        if filetype == "TelescopePrompt" then return require("cmp_im").getStatus() end
        return true
      end,
      completion = {
    ...
    cmp.setup.cmdline({ "/", "?" }, {
      mapping = cmp.mapping.preset.cmdline {
        ["<Space>"] = {
          c = function(fallback)
            ...
            if cmp_im.getStatus() and cmp.visible() then
              ...
              if #entries > 0 and entries[1].source.name == "IM" then return cmp.confirm { select = true } end
            end
            return fallback()
          end,
        },
        ["<End>"] = {
          c = function(fallback)
            local cmp_im = require "cmp_im"
            if cmp_im.getStatus() and cmp.visible() then return cmp.abort() end
            return fallback()
          end,
        },
      },
